...
void internal_sha1_core(sha1_context* context, unsigned char* buffer)
{
	if (context && buffer) {
		...
		memcpy((unsigned char*)block, buffer, 64);
...
void sha1_update(sha1_context* context, unsigned char* buffer, unsigned int length) {
	if (context && buffer && length > 0) {
		...
		memcpy(&context->buffer[j], &buffer[i], (length - i));
	}
}
...
void sha1_final(sha1_context* context, unsigned char* result) {
	if (context && result) {
		...
		for (unsigned int i = 0; i < 8; i++) {
			f[i] = (unsigned char)(context->count[(i >= 4 ? 0 : 1)] >> ((3 - (i & 3)) * 8) & 255);
		}
		...
		for (unsigned int i = 0; i < 20; i++) {
			result[i] = (unsigned char)((context->state[i >> 2] >> ((3 - (i & 3)) * 8)) & 255);
		}

		//wipe
		memset(context->buffer, 0, 64);
		memset((unsigned char*)context->state, 0, 20);
		memset((unsigned char*)context->count, 0, 8);
		memset(f, 0, 8);
